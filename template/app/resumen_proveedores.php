...
<div class="br-pagebody">
  <div class="br-section-wrapper">
    ...
    <div class="header">
        ...
        <legend>Listado de proveedores
        <a href="" class="btn btn-info btn-with-icon pull-right" href="" data-toggle="modal" data-target="#modal_proveedor" onclick="limpiar_proveedor()">
...
<div id="modal_proveedor" class="modal fade">
  
  <div class="modal-dialog modal-dialog-vertical-center modal-lg" role="document" style=" max-width: 1200px;">
    <div class="modal-content bd-0 tx-14">
      <form id="guardar_proveedor" method="post" enctype="multipart/form-data" > 
      <div class="modal-header pd-y-20 pd-x-25">
        <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">Proveedor</h6>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body pd-25" id="div_modal_proveedor">
...
<div id="modalCancelar" class="modal fade">
  <div class="modal-dialog modal-dialog-vertical-center" role="document">
    <div class="modal-content bd-0 tx-14">
      <form id="guardar_ubicacion" method="post" enctype="multipart/form-data"> 
      <div class="modal-header pd-y-20 pd-x-25">
        <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">CANCELAR PEDIDO</h6>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body pd-25" id="div_modal_ubicacion">
       <p class="mg-b-5">¿Desea cancelar el pedido completo?</p>
      </div>
      <div class="modal-footer">
        
        <button type="button" class="btn btn-secondary tx-11 tx-uppercase pd-y-12 pd-x-25 tx-mont tx-medium" data-dismiss="modal">Cerrar</button>
        <input id="guardar" type="submit" name="" class="btn btn-primary tx-11 tx-uppercase pd-y-12 pd-x-25 tx-mont tx-medium" value="Cancelar"> 
      </div>
...
$(document).ready (function() {
    $.ajax({
        url: "../backend/get_proveedores.php",
        beforeSend: function(){
              //imagen de carga
            ...
            table.on( 'click', '.delete', function (e) {
              ...
              if (confirma == true){
                  ...
                  var list_id = data[0];
                   ...
                   $('#procesa_proveedor').load('../backend/guardar_proveedor.php?id_borrar='+list_id);
             ...
             table.on( 'click', '.modify', function (e) {
                ...
                document.getElementById("div_modal_proveedor").innerHTML = "";
                
                $('#div_modal_proveedor').load('edita_proveedor.php?id='+list_id);
...
$('#guardar_proveedor').submit(function () {

   $('#modal_proveedor').modal('hide');

    $('#procesa_proveedor').delay(800).queue(function(nxt){   
      var formData = new FormData($("#guardar_proveedor")[0]);
      $.ajax({
        type: 'POST',
        url: '../backend/guardar_proveedor.php',
        data: formData,
        contentType: false,
        processData: false,
        beforeSend: function(){
         
          //$("#procesa_proveedor").html("<p align='center'><img src='../ajax-loader.gif' /></p>");
        },
        success: function(data){
          $("#procesa_proveedor").empty();
          $("#procesa_proveedor").append(data);
        }
      });
    });   
...
function limpiar_proveedor(){
    ...
    //document.getElementById("tab-content-proveedor").value = "" ;

    $("#tab-content-proveedor").empty();
